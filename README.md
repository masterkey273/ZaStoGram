...
- `Auto rotate` — стабильный профиль для endpoint, но после подозрительного
  FakeTLS-сбоя клиент переключает endpoint на другой профиль из безопасного
...
набор параллельных download/upload connection type для каждого запроса. Они
мягко сводятся к основному download/upload каналу, а прямые соединения без
...
В настройках списка прокси слой называется `Ограничивать старт FakeTLS`. По
умолчанию он выключен: базовый путь остаётся максимально близким к рабочему
транспорту, а admission-controller включается вручную для A/B-тестов. Переключатель
сохраняется в `SharedConfig`, передаётся в native через `setProxySettings` и
перезапускает соединения при изменении.

Когда слой включён, его задача:
